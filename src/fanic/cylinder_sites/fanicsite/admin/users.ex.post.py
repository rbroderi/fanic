...
def main(request: RequestLike, response: ResponseLike) -> ResponseLike:
    ...
    if action == "create":
        ...
        if (
            role_value == ManagedUserRole.SUPERADMIN.value
    ...
    if (
        target_user["role"] == ManagedUserRole.SUPERADMIN.value
        and actor_role != ManagedUserRole.SUPERADMIN.value
    ):
        return _redirect_msg(response, "forbidden-action")
    ...
    if (
        ...
        and target_username == actor_username
        and action in {"set-active", "remove"}
    ):
