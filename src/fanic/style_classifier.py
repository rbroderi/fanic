...
def _call_kw(
    obj: object | None, name: str, *args: object, **kwargs: object
) -> object | None:
    if obj is None:
        return None
    member = getattr(obj, name, None)
...
def _ensure_loaded() -> bool:
    ...
    try:
        ...
        if created_model is None:
            ...
            progress.close()
            _last_load_error = "create_model_and_transforms returned invalid result"
        ...
        if _tokenizer is None:
            _LOGGER.warning(
                "Style tokenizer initialization failed for model=%s", _MODEL_NAME
            )
            progress.close()
            _last_load_error = "get_tokenizer returned None"
            _last_load_failed_at = time.time()
            return False
        _ = progress.update(1)
        ...
        with no_grad:
            ...
            for style_name, start, end in style_ranges:
                _ = style_name
                prompt_slice = _call1(
                    prompt_text_emb_normed, "__getitem__", slice(start, end)
                )
    ...
    try:
        ...
        with no_grad:
            ...
            if len(sorted_probs) >= 2:
                ...
                if (
                    sorted_probs[0] < _STYLE_MIN_TOP_PROB
                    or top_gap < _STYLE_MIN_TOP_MARGIN
                ):
            ...
            if predicted_style == "photorealistic":
                ...
                if top_prob is not None:
                    drawn_probs = [
                        confidence
                        for style_name, confidence in confidences.items()
            ...
            if (
                min_confidence > 0
                and top_prob is not None
                and top_prob < min_confidence
            ):
...
def get_style_classifier_debug_state() -> dict[str, object]:
    return {
        ...
        "device": _device,
        "model_loaded": bool(
            _model is not None and _preprocess is not None and _text_emb is not None
        ),
