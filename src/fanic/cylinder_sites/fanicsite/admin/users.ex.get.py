...
def _role_options_html(selected_role: str) -> str:
    ...
    for role in ManagedUserRole:
        ...
        options.append(
            f'<option value="{escape(role.value)}"{selected_attr}>{escape(role.label())}</option>'
        )
    return "".join(options)


def _users_rows_html(
    users: Sequence[LocalUserRow], *, actor_username: str, actor_role: str
) -> str:
    if not users:
    ...
    for user in users:
        ...
        if (
            role == ManagedUserRole.SUPERADMIN.value
            and actor_role != ManagedUserRole.SUPERADMIN.value
        ):
