...
def main(request: RequestLike, response: ResponseLike) -> ResponseLike:
    ...
    issue_options_html = (
        feedback_category_options_html(issue_type)
        if tab == "feedback"
        else report_issue_options_html(issue_type)
    )
    ...
    return render_html_template(
        ...
        "reports.html",
        {
            ...
            "__REPORT_TAB_CONTENT_ACTIVE__": (
                'aria-current="page"' if tab == "content" else ""
            ),
            "__REPORT_TAB_FEEDBACK_ACTIVE__": (
                'aria-current="page"' if tab == "feedback" else ""
            ),
            "__REPORT_TITLE__": (
                "Feedback Queue" if tab == "feedback" else "Submitted Reports"
            ),
