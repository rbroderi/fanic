...
def test_content_addressed_storage_is_idempotent(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ingest = _load_ingest_with_stubs(monkeypatch, tmp_path)
...
def test_small_conversion_helpers(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ingest = _load_ingest_with_stubs(monkeypatch, tmp_path)
...
def test_rating_normalization_and_elevation(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ingest = _load_ingest_with_stubs(monkeypatch, tmp_path)
...
def test_parse_comicinfo_xml_extracts_fanic_extended_fields(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ingest = _load_ingest_with_stubs(monkeypatch, tmp_path)
    ...
    xml_text = """
        <ComicInfo>
            <Title>My Comic</Title>
            <Writer>Artist One, Writer Two</Writer>
            <Genre>Romance, Adventure</Genre>
            <Series>Zootopia</Series>
            <StoryArc>Nick/Judy</StoryArc>
            <SeriesGroup>Skyrim</SeriesGroup>
            <Review>Graphic Violence</Review>
            <Notes>freeform_tags=freeform-tag</Notes>
            <AgeRating>Teen</AgeRating>
            <ScanInformation>fanic_meta={"id":"work123","slug":"my-comic","status":"complete","cover_page_index":3,"creators":["Artist One","Writer Two"]}</ScanInformation>
            <Pages>
    ...
    assert parsed["creators"] == ["Artist One", "Writer Two"]
    assert parsed["fandoms"] == ["Zootopia", "Skyrim"]
    assert parsed["relationships"] == ["Nick/Judy"]
    assert parsed["categories"] == ["Romance", "Adventure"]
    assert parsed["warnings"] == ["Graphic Violence"]
    assert parsed["freeform_tags"] == ["freeform-tag"]
    assert parsed["id"] == "work123"
    assert parsed["slug"] == "my-comic"
    assert parsed["status"] == "complete"
    assert parsed["cover_page_index"] == 3
    assert parsed["rating"] == "Teen And Up Audiences"


def test_image_detection_helpers(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ingest = _load_ingest_with_stubs(monkeypatch, tmp_path)
...
def test_chapter_seed_members_from_range(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ingest = _load_ingest_with_stubs(monkeypatch, tmp_path)
    ...
    seeded_fallback = ingest._chapter_seed_members_from_range(page_order, out_of_bounds_map)
...
def test_ingest_cbz_success_with_override_and_progress(
    ...
    def moderate_image_bytes_stub(data: bytes, suffix: str = ".png") -> dict[str, object]:
        _ = (data, suffix)
        return {
            "allow": True,
            "style": "comic",
...
def test_ingest_cbz_creates_chapters_from_member_directories(
    ...
    def render_image_bytes_stub(image: object, fmt: str, quality: int) -> bytes:
        ...
        return f"avif-{rendered_counter['value']}".encode()
...
def test_editor_move_page_reorders_and_snapshots(
    ...
    monkeypatch.setattr(ingest, "_reconcile_chapters_after_page_changes", reconcile_stub)
...
def test_editor_reorder_gallery_validates_and_updates_chapters(
    ...
    chapter_rows: list[dict[str, object]] = [{"id": 7, "title": "Ch1", "start_page": 1, "end_page": 2}]
    ...
    with pytest.raises(ValueError, match="Ordered page list does not match existing pages"):
...
def test_reconcile_chapters_updates_and_deletes(
    ...
    ingest._reconcile_chapters_after_page_changes("w1", removed_image_filename="removed")
