...
def _normalize_rating(value: str) -> str:
    ...
    rating_aliases: dict[str, str] = {
        ...
        "everyone 10+": "General Audiences",
        "kids to adults": "General Audiences",
        ...
        "rating pending": "Teen And Up Audiences",
...
def parse_comicinfo_xml(xml_text: str) -> ComicInfoMetadata:
    ...
    creators = _comma_split(_clean_xml_value(root.findtext("Writer")))
    if creators:
    ...
    categories = _comma_split(_clean_xml_value(root.findtext("Genre")))
    if categories:
        metadata["categories"] = categories

    story_arc_values = _comma_split(_clean_xml_value(root.findtext("StoryArc")))
    if story_arc_values:
        metadata["relationships"] = _dedupe_preserve_order(story_arc_values)

    series_group_values = _comma_split(_clean_xml_value(root.findtext("SeriesGroup")))
    if series_group_values:
        existing_fandoms = metadata.get("fandoms", [])
        if isinstance(existing_fandoms, list):
            metadata["fandoms"] = _dedupe_preserve_order([*existing_fandoms, *series_group_values])
        else:
            metadata["fandoms"] = _dedupe_preserve_order(series_group_values)

    notes_value = _clean_xml_value(root.findtext("Notes"))
    if notes_value:
        notes_parts = [part.strip() for part in notes_value.split(";") if part.strip()]
        for part in notes_parts:
            if "=" not in part:
                continue
            key, value = part.split("=", 1)
            normalized_key = key.strip().lower()
            parsed_values = _comma_split(value)
            if normalized_key == "freeform_tags" and parsed_values:
                metadata["freeform_tags"] = _dedupe_preserve_order(parsed_values)
            if normalized_key == "warnings" and parsed_values:
                metadata["warnings"] = _dedupe_preserve_order(parsed_values)

    review_warnings = _comma_split(_clean_xml_value(root.findtext("Review")))
    if review_warnings:
        existing_warnings = metadata.get("warnings", [])
        if isinstance(existing_warnings, list):
            metadata["warnings"] = _dedupe_preserve_order([*existing_warnings, *review_warnings])
        else:
            metadata["warnings"] = _dedupe_preserve_order(review_warnings)

    main_character = _clean_xml_value(root.findtext("MainCharacterOrTeam"))
    if main_character:
        existing_characters = metadata.get("characters", [])
        if isinstance(existing_characters, list):
            metadata["characters"] = _dedupe_preserve_order([main_character, *existing_characters])
        else:
            metadata["characters"] = [main_character]

    tags = _comma_split(_clean_xml_value(root.findtext("Tags")))
    freeform_tags: list[str] = []
    tag_fandoms: list[str] = []
    tag_relationships: list[str] = []
    tag_categories: list[str] = []
    tag_warnings: list[str] = []
    for raw_tag in tags:
        lower_raw_tag = raw_tag.lower()
        if lower_raw_tag.startswith("fandom:"):
            value = raw_tag.split(":", 1)[1].strip()
            if value:
                tag_fandoms.append(value)
            continue
        if lower_raw_tag.startswith("relationship:"):
            value = raw_tag.split(":", 1)[1].strip()
            if value:
                tag_relationships.append(value)
            continue
        if lower_raw_tag.startswith("category:"):
            value = raw_tag.split(":", 1)[1].strip()
            if value:
                tag_categories.append(value)
            continue
        if lower_raw_tag.startswith("warning:"):
            value = raw_tag.split(":", 1)[1].strip()
            if value:
                tag_warnings.append(value)
            continue
        freeform_tags.append(raw_tag)

    if freeform_tags:
        metadata["freeform_tags"] = _dedupe_preserve_order(freeform_tags)
    if tag_fandoms:
        metadata["fandoms"] = _dedupe_preserve_order(tag_fandoms)
    if tag_relationships:
        metadata["relationships"] = _dedupe_preserve_order(tag_relationships)
    if tag_categories:
        metadata["categories"] = _dedupe_preserve_order(tag_categories)
    if tag_warnings:
        metadata["warnings"] = _dedupe_preserve_order(tag_warnings)
    ...
    scan_information = _clean_xml_value(root.findtext("ScanInformation"))
    if scan_information:
        scan_json = ""
        if scan_information.startswith("fanic_meta="):
            scan_json = scan_information.split("=", 1)[1].strip()
        if scan_json:
            try:
                parsed_json = json.loads(scan_json)
            except (TypeError, ValueError):
                parsed_json = None
            if isinstance(parsed_json, Mapping):
                parsed_meta = cast(Mapping[object, object], parsed_json)
                meta_id = str(parsed_meta.get("id", "") if parsed_meta.get("id", "") else "").strip()
                if meta_id:
                    metadata["id"] = meta_id
                meta_slug = str(parsed_meta.get("slug", "") if parsed_meta.get("slug", "") else "").strip()
                if meta_slug:
                    metadata["slug"] = meta_slug
                meta_status = str(parsed_meta.get("status", "") if parsed_meta.get("status", "") else "").strip()
                if meta_status:
                    metadata["status"] = meta_status
                meta_cover = parsed_meta.get("cover_page_index", 0)
                meta_cover_value = _as_int(meta_cover, 0)
                if meta_cover_value > 0:
        ...
        fanic_fields = _parse_fanic_scan_information(scan_information)
        status = fanic_fields.get("fanic_status", "")
        if status:
            metadata["status"] = status
        work_id = fanic_fields.get("fanic_id", "")
        if work_id:
            metadata["id"] = work_id
        slug = fanic_fields.get("fanic_slug", "")
        if slug:
            metadata["slug"] = slug
        cover_page_index = fanic_fields.get("fanic_cover_page_index", "")
        if cover_page_index and cover_page_index.isdigit():
            metadata["cover_page_index"] = int(cover_page_index)

    pages_node = root.find("Pages")
    if pages_node is not None:
        for page_node in pages_node.findall("Page"):
            page_type = str(page_node.attrib.get("Type", "") if page_node.attrib.get("Type", "") else "").strip()
            if page_type != "FrontCover":
                continue
            image_value = str(page_node.attrib.get("Image", "") if page_node.attrib.get("Image", "") else "").strip()
            if image_value and image_value.isdigit():
                metadata["cover_page_index"] = int(image_value) + 1
                break

    series_fandoms = _comma_split(series)
    if series_fandoms:
        existing_fandoms = metadata.get("fandoms", [])
        if isinstance(existing_fandoms, list):
            metadata["fandoms"] = _dedupe_preserve_order([*series_fandoms, *existing_fandoms])
        else:
            metadata["fandoms"] = _dedupe_preserve_order(series_fandoms)

    existing_categories = metadata.get("categories", [])
    if categories and isinstance(existing_categories, list):
        metadata["categories"] = _dedupe_preserve_order([*categories, *existing_categories])
...
def ingest_cbz(
    ...
    *,
    ...
    with ZipFile(cbz_path) as zip_file:
        ...
        metadata: dict[str, object] = {key: value for key, value in _load_metadata_from_comicinfo(zip_file).items()}
        if metadata_override_path:
            override_obj = json.loads(metadata_override_path.read_text(encoding="utf-8"))
        ...
        image_members = [member for member in candidate_members if _is_image_member_name(member)]
        ...
        if len(image_members) > MAX_INGEST_PAGES:
            raise ValueError(f"CBZ exceeds maximum allowed page count ({len(image_members)} > {MAX_INGEST_PAGES})")
        ...
        if configured_order:
            image_members = sorted(
                image_members,
                key=lambda name: (
                    configured_order.index(Path(name).name) if Path(name).name in configured_order else 999999,
    ...
    cover_page_index = int(cover_page_raw) if isinstance(cover_page_raw, (int, str)) else 1
...
def ingest_editor_page(
    ...
    if existing_work:
        existing_uploader = str(
            existing_work.get("uploader_username") if existing_work.get("uploader_username") else ""
        )
    ...
    title = str(
        ...
        else (existing_work.get("title") if existing_work and existing_work.get("title") else image_path.stem)
    )
    slug = str(
    ...
    cover_page_index_raw = existing_work.get("cover_page_index", 1) if existing_work else 1
    ...
    work: dict[str, object] = {
        ...
        "warnings": str(
            ...
            else (existing_work.get("warnings") if existing_work else "No Archive Warnings Apply")
        ),
        "language": str(
        ...
        "uploader_username": str(
            existing_work.get("uploader_username") if existing_work.get("uploader_username") else uploader_username
        )
        if existing_work
...
def _upsert_existing_work(existing_work: dict[str, object], pages: list[WorkPageRow]) -> None:
...
def _require_editor_owner(work_id: str, uploader_username: str) -> dict[str, object]:
    ...
    existing_uploader = str(existing_work.get("uploader_username") if existing_work.get("uploader_username") else "")
    if existing_uploader and existing_uploader != uploader_username:
...
def editor_replace_page_image(
    ...
    for chapter in chapters:
        chapter_id = _as_int(chapter.get("id", 0), 0)
        members = list_work_chapter_members(chapter_id)
        remapped_members = [image_name if name == old_image_name else name for name in members]
...
def editor_move_page(
    ...
    existing_work = _require_editor_owner(work_id, uploader_username)
    pages = list_work_page_rows(work_id)
    if from_index < 1 or from_index > len(pages) or to_index < 1 or to_index > len(pages):
...
def editor_add_chapter(
    ...
    result = add_work_chapter(work_id, title=title, start_page=start_page, end_page=end_page)
    _ = create_work_version_snapshot(
        work_id,
...
def editor_delete_chapter(work_id: str, chapter_id: int, uploader_username: str) -> bool:
    _ = _require_editor_owner(work_id, uploader_username)
