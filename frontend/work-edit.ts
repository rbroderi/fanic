...
(function () {
  ...
  const galleryRoot = document.getElementById("editorPageGallery") as HTMLElement | null;
  if (!galleryRoot) {
    return;
  }
  const gallery = galleryRoot;

  const selectedPageLabel = document.getElementById("selectedPageLabel") as HTMLElement | null;
  const insertAfterPageIndex = document.getElementById(
    "insertAfterPageIndex",
  ) as HTMLInputElement | null;
  const replacePageIndex = document.getElementById("replacePageIndex") as HTMLInputElement | null;
  const deletePageIndex = document.getElementById("deletePageIndex") as HTMLInputElement | null;
  const replaceSelectedButton = document.getElementById(
    "replaceSelectedButton",
  ) as HTMLButtonElement | null;
  const deleteSelectedButton = document.getElementById(
    "deleteSelectedButton",
  ) as HTMLButtonElement | null;
  const orderedFilenamesInput = document.getElementById(
    "orderedFilenamesInput",
  ) as HTMLInputElement | null;
  const chapterMembersInput = document.getElementById(
    "chapterMembersInput",
  ) as HTMLInputElement | null;
  const saveGalleryOrderButton = document.getElementById(
    "saveGalleryOrderButton",
  ) as HTMLButtonElement | null;
  const galleryDirtyBadge = document.getElementById("galleryDirtyBadge") as HTMLElement | null;
  ...
  function refreshActionLockState() {
    const selected = gallery.querySelector(".page-thumb-card.selected");
  ...
  function captureGalleryState() {
    ...
    const chapterSections = Array.from(
      gallery.querySelectorAll<HTMLElement>(".chapter-gallery-section"),
    );
    ...
    chapterSections.forEach((section) => {
      ...
      const members = Array.from(section.querySelectorAll<HTMLElement>(".page-thumb-card")).map(
        (card) => card.dataset.imageFilename || "",
      );
  ...
  function bindDragAndDrop(card: HTMLElement) {
  ...
  const grids = Array.from(gallery.querySelectorAll<HTMLElement>(".page-gallery-grid"));
  grids.forEach((grid) => {
